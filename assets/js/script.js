...
function showPosition(position) {
    ...
    // Use Google Maps or Leaflet to display the location
...
var heartRateRef = firebase.database().ref('/heartRate');
heartRateRef.on('value', (snapshot) => {
    var heartRate = snapshot.val();
    $('#heart_rate').text(heartRate);

    // Check for low heart rate and send notification
    if (heartRate < 50) {
        sendNotification("Warning: Heart rate is too low!");
    }
});
